...
def initialize_parameters():
    ...
    W1 = tf.get_variable("W1", [8, 8, 13, 8], initializer=tf.contrib.layers.xavier_initializer(seed=0))
...
def forward_propagation(X, parameters):
    ...
    P2 = tf.contrib.layers.dropout(P2, keep_prob=0.5, is_training=True)
...
def model(X_train, Y_train, X_test, Y_test, learning_rate=0.001,
    ...
    with tf.Session() as sess:
        ...
        train_accuracy = accuracy.eval({X: X_train, Y: Y_train, phase: True})
        test_accuracy = accuracy.eval({X: X_test, Y: Y_test, phase: False})
